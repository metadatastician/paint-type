...
| TP-2 | Public API type safety (exported pt_ functions) | **Done** | `verification/proofs/lean4/ApiTypes.lean` |
...
| # | Proof | Status | File |
|---|-------|--------|------|
| INV-1 | Tile pool invariant (no double-free, no use-after-free) | **Done** | `verification/proofs/idris2/TilePool.idr` |
| INV-2 | Undo graph monotonicity (history only grows; no silent discard) | **Done** | `verification/proofs/lean4/UndoGraph.lean` |
| INV-3 | Compositing blend function totality (terminates on all inputs) | **Done** | `verification/proofs/agda/Compositing.agda` |
...
| # | Proof | Status | File |
|---|-------|--------|------|
| CONC-1 | CRDT tile merge commutativity (A⊕B = B⊕A) | **Done** | `verification/proofs/agda/TileCRDT.agda` |
| CONC-2 | CRDT tile merge associativity (A⊕(B⊕C) = (A⊕B)⊕C) | **Done** | `verification/proofs/agda/TileCRDT.agda` |
| CONC-3 | Session liveness (every committed tile mutation is eventually visible) | **Done** | `verification/proofs/tlaplus/BurbleSession.tla` |
