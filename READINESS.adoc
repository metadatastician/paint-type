...
**Current Grade:** D (reconciled 2026-09-22 — lowered to match the CRG-D badge in README.adoc; pending a fresh assessment)
**Assessed:** 2026-07-25
...
| Zig FFI (libpt)     | C     | Pre-alpha     | 23/23 tests pass; 23 exports (pt_tile_* + pt_layer_* + slot helpers) |
| Ephapax (Rust core) | C     | Pre-alpha     | Tile API + 11 compositing ops + UndoGraph + layer model + brush engine + benches (cargo test 104/104 + 1 doctest) |
...
- `src/interface/ffi/` — Zig libpt builds and integration tests pass (23/23)
- `src/paint_core/` — Rust crate builds with `cargo test` (104/104 + 1 doctest); `cargo clippy --all-targets -- -D warnings` clean
...
- OPERATIONAL-STATUS.adoc, TEST-NEEDS.adoc, PROOF-NEEDS.adoc, and ROADMAP.adoc reflect actual project state
...
- `cargo test` **104/104** + 1 doctest pass (lib + composite + undo +
  layer + brush modules) after the f16→f32 underflow fix (PR #11).
- `zig build && zig build test` **23/23** pass — pt_tile_* lifecycle/fill/
