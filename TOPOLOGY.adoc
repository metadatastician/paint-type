...
| AffineScript bridge | AffineScript → typed-wasm | High-level API surface for UI and plugins. Verified `.twasm` schemas at `src/bridges/paint-type-{tile,layer}.twasm`; verified by typed-wasm (paint-type#39 closed). |
