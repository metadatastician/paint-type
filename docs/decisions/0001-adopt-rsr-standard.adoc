...
Adopt the Rhodium Standard Repository (RSR) template (`paint-type`) as
...
- **Security and governance files**: SECURITY.md, CONTRIBUTING.md,
  CODE_OF_CONDUCT.md, LICENSE (AGPL-3.0-or-later with PMPL-1.0-or-later fallback)
- **Architecture Decision Records** in `docs/decisions/`

New repositories are created by cloning the template:

```bash
git clone https://github.com/metadatastician/paint-type new-repo-name
cd new-repo-name
rm -rf .git && git init
```
