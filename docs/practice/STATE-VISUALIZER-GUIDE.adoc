...
= OPERATIONAL-STATUS / TOPOLOGY — Generation Guide
Joshua Jewell (JoshuaJewell) <paint-type@pm.me>
:toc:
:sectnums:

== What Is OPERATIONAL-STATUS / TOPOLOGY?
...
[source,text]
----
Read the entire repository and produce an OPERATIONAL-STATUS.adoc (or TOPOLOGY.adoc) file for the repo root.
...
Use the template at OPERATIONAL-STATUS.adoc in paint-type as a structural reference,
...
[source,bash]
----
cp /path/to/paint-type/OPERATIONAL-STATUS.adoc ./OPERATIONAL-STATUS.adoc
...
for repo in /path/to/your/repos/*/; do
  if [ ! -f "$repo/OPERATIONAL-STATUS.adoc" ] && [ ! -f "$repo/TOPOLOGY.adoc" ]; then
    echo "NEEDS OPERATIONAL-STATUS/TOPOLOGY: $(basename $repo)"
  fi
done
----
...
OPERATIONAL-STATUS/TOPOLOGY complements but does not replace:
...
OPERATIONAL-STATUS/TOPOLOGY is the _visual summary_ for humans; the a2ml files are the
